...
// 全链端到端冒烟（超管一条龙，对齐后端 EndToEndSmokeTest 思路）：
// 基础数据(供应商/物流商) → 供应商产品(零售价 200) → 采购计划(3) → 提交 → 审批通过 →
// 采购订单付款 → 确认(生成实际采购单) → 入库(库存=3) → 销售下单(3) →
// 物流派送 → 签收 → 拒收 1 → 完成。
// 断言关键终值：订单实收 actualAmount = 200×(3-1) = 400；库存终值 = 3-3+1 = 1。
...
async function pick(page: Page, selector: string, title: string) {
  await page.locator(selector).click();
  await page.locator(`.ant-select-item-option[title="${title}"]`).click();
}

test('全链冒烟：采购→入库→销售→物流完成，断言实收与库存终值', async ({ page }) => {
  ...
  await pick(page, '[data-test="cascade-supplier"] .ant-select-selector', supplierName);
  ...
  await pick(page, '[data-test="plan-filter-supplier"] .ant-select-selector', supplierName);
  ...
  await pick(page, '[data-test="order-filter-supplier"] .ant-select-selector', supplierName);
  ...
  await page.locator('[data-test="cascade-search"]').click();
  await drawer.getByRole('spinbutton').first().fill('3');
  const cust = drawer.locator('.ant-card:has-text("客户信息") input');
  await cust.nth(0).fill('Tom');
  await cust.nth(1).fill('0240000000');
  await cust.nth(2).fill('Accra');
  ...
  await pick(page, '[data-test="dispatch-provider"] .ant-select-selector', providerName);
  ...
  // 库存终值：入库 3 - 售 3 + 拒收回补 1 = 1
  await page.goto('/inventory/stock');
  await pick(page, '[data-test="cascade-supplier"] .ant-select-selector', supplierName);
  await page.locator('[data-test="cascade-search"]').click();
  await expect(page.getByRole('row', { name: new RegExp(`${productName}.*\\b1\\b`) })).toBeVisible();
});
